Exécution de: cat /jevoispro/share/pydnn/post/PyPostYolov7ColorTrackerFiltered.py
import sys
...
#
class PyPostYolov7ColorTrackerFiltered:
    ...
    def process(self, outs, preproc):
        ...
        try:
            ...
            if len(boxes) > 0:
                ...
                for i in indices:
                    # FILTER: Only keep person and vehicles
                    if classids[i] not in [0, 1, 2, 3, 5, 7]:
                        continue
                    x, y, w, h = boxes[i]
                    x1 = min(bw - 1, max(0, x))
